// Centralized ignore-glob matcher. Ported from ui-tokenize/lib/ignore.mjs verbatim;
// the only difference is the hard-default added for `.responsive/` instead of `.tokenize/`.
...
/**
 * Build an IgnoreMatcher rooted at `base`. Reads `.gitignore` and
 * `.responsive/ignore` if present; appends `extra` patterns; always applies
 * HARD_DEFAULTS.
 *
 * @param {string} base
 * @param {string[]} [extra]
...
export function loadIgnore(base, extra = []) {
  ...
  for (const file of [join(base, '.gitignore'), join(base, '.responsive', 'ignore')]) {
    if (existsSync(file)) {
      try {
 ...
 * audit `--suppressions`) without building a full IgnoreMatcher.
 *
 * @param {string} p
